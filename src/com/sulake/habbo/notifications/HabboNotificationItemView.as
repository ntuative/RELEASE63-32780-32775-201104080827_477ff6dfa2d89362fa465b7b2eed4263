package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements IUpdateReceiver
   {
      ...
      private static const const_507:int = 1;
      
      private static const const_966:int = 2;
      
      private static const const_965:int = 3;
      
      private static const const_432:Number = 50;
      ...
      public function HabboNotificationItemView(param1:IAsset, param2:IHabboWindowManager, param3:Map, param4:Map, param5:IHabboToolbar, param6:HabboNotificationItem)
      {
         ...
         if(_loc8_ != null)
         {
            this.var_1712 = this._window.height - _loc8_.rectangle.bottom;
         }
         else
         {
            this.var_1712 = 15;
         }
         this.var_1710 = this._window.height;
         this.var_739 = 4;
         this.var_740 = 0;
         this._state = name_3;
         this.var_160 = param5;
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(this._state)
         {
            ...
            case const_507:
               ...
               _loc2_ = Number(this.var_1132) / Number(this.var_418["time_fade_in"]);
               if(this.var_1132 > int(this.var_418["time_fade_in"]))
               {
                  this.startDisplay();
               }
            ...
            case const_966:
               ...
               if(this.var_1399 > int(this.var_418["time_display"]) && !this.var_1711)
               {
                  this.startFadeOut();
               }
               break;
            case const_965:
               ...
               _loc3_ = 1 - Number(this.var_1133) / Number(this.var_418["time_fade_out"]);
               this.adjustBlend(_loc3_);
               if(this.var_1133 > int(this.var_418["time_fade_in"]))
               {
                  this.startIdling();
               }
         }
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         _loc2_.text = param1;
         _loc2_.height = _loc2_.textHeight + this.var_1712;
         if(this._window.height < this.var_1710)
         {
            this._window.height = this.var_1710;
         }
      }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = this.var_160.orientation == "TOP" ? int(const_432) : 0;
         var _loc4_:int = this.var_160.orientation == "BOTTOM" ? int(_loc2_.height - const_432) : int(_loc2_.height);
         var _loc5_:int = this.var_160.orientation == "LEFT" ? int(const_432) : 0;
         var _loc6_:int = this.var_160.orientation == "RIGHT" ? int(_loc2_.width - const_432) : int(_loc2_.width);
         if(this.var_418["position_horizontal_follow_toolbar"] == "true" && (this.var_160.orientation != "TOP" && this.var_160.orientation != "BOTTOM"))
         {
            if(this.var_160.orientation == "LEFT")
            {
         ...
         if(this.var_418["position_vertical_follow_toolbar"] == "true" && (this.var_160.orientation != "RIGHT" && this.var_160.orientation != "LEFT"))
         {
            if(this.var_160.orientation == "TOP")
            {
               this._window.y = _loc3_ + this.var_739;
            }
            else
            {
               this._window.y = _loc4_ - this._window.height - this.var_739;
            }
         }
      ...
      private function adjustBlend(param1:Number) : void
      {
         ...
         this._window.blend = this.var_740;
      }
   }
}
