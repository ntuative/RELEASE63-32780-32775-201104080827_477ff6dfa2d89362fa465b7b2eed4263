package com.sulake.habbo.avatar.animation
{
   ...
   public class Animation implements IAnimation
   {
      ...
      private var var_1994:AvatarDataContainer;
      
      private var var_1542:Array;
      
      private var var_1993:DirectionDataContainer;
      
      private var var_1992:ShadowDataContainer;
      ...
      public function Animation(param1:AvatarStructure, param2:XML)
      {
         ...
         {
            this.var_1825 = String(param2.@desc);
         }
         else
         {
            this.var_1825 = this._id;
         }
         if(param2.hasOwnProperty("sprite"))
         {
            for each(_loc3_ in param2.sprite)
            {
               this.var_1543.push(new SpriteDataContainer(this as IAnimation,_loc3_));
            }
         }
         if(param2.hasOwnProperty("avatar"))
         {
            this.var_1994 = new AvatarDataContainer(param2.avatar[0]);
         }
         if(param2.hasOwnProperty("canvas"))
         {
            for each(_loc4_ in param2.canvas)
            {
               this.var_1542.push(new CanvasDataContainer(_loc4_));
            }
         }
         if(param2.hasOwnProperty("direction"))
         {
            this.var_1993 = new DirectionDataContainer(param2.direction[0]);
         }
         if(param2.hasOwnProperty("shadow"))
         {
            this.var_1992 = new ShadowDataContainer(param2.shadow[0]);
         }
         ...
         if(param2.hasOwnProperty("PetLevelNotificationEvent"))
         {
            this._overrideFrames = new Map();
            this.var_666 = new Map();
            for each(_loc7_ in param2.PetLevelNotificationEvent)
            {
               _loc8_ = _loc7_.@name;
               _loc9_ = _loc7_.@PetLevelNotificationEvent;
               this.var_666.add(_loc9_,_loc8_);
               _loc10_ = new Array();
      ...
      private function parseFrames(param1:Array, param2:XMLList, param3:AvatarStructure) : void
      {
         ...
         for each(_loc7_ in param2)
         {
            ...
            for each(_loc9_ in _loc7_.fx)
            {
               ...
               _loc11_ = new AnimationLayerData(param3,_loc9_,AnimationLayerData.const_502,_loc6_,_loc5_);
               _loc4_.push(_loc11_);
               if(_loc5_ != null)
               {
               }
      ...
      public function getAnimatedBodyPartIds(param1:int, param2:String = null) : Array
      {
         ...
         for each(_loc4_ in this.getFrame(param1,param2))
         {
            ...
            else if(_loc4_.type == AnimationLayerData.const_502)
            {
      ...
      public function getLayerData(param1:int, param2:String, param3:String = null) : AnimationLayerData
      {
         ...
         for each(_loc4_ in this.getFrame(param1,param3))
         {
            ...
            if(_loc4_.type == AnimationLayerData.const_502)
            {
               for each(_loc5_ in this.var_826)
               {
