package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      public function removeListener(param1:IPetDataListener = null) : void
      {
         ...
         var _loc2_:int = this.var_477.indexOf(param1);
         if(_loc2_ != -1)
         {
            this.var_477.splice(_loc2_,1);
         }
      }
      ...
      {
         var _loc1_:* = null;
         while(this.var_477.length > 0)
         {
            _loc1_ = this.var_477.pop() as IPetDataListener;
            ...
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.var_2203.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1330.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_572.getValue(_loc6_) == null)
               {
                  ...
                  this.var_572.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_477.indexOf(param3) == -1)
                  {
                     this.var_477.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_572.getKeys())
         {
            _loc5_ = this.var_572.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
