package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_1795:int = 60000;
       
      
      private var _structure:AvatarStructure;
      
      private var var_1407:IAvatarImage;
      ...
      private var var_411:AvatarImageBodyPartContainer;
      ...
      private var var_2714:ByteArray;
      
      private var var_2715:ImageData;
      
      private var var_937:PaletteMap;
      
      private var var_1014:Array;
      
      private var var_1016:Point;
      ...
      {
         if(this.var_815 == param1)
         {
            return;
         }
         if(this.var_815 == GeometryType.const_1050 && param1 == GeometryType.const_1131 || this.var_815 == GeometryType.const_1131 && param1 == GeometryType.const_1050)
         {
            this.var_815 = param1;
            this.var_86 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
            ...
            if(this.var_385.definition.isAnimation)
            {
               ...
               if(_loc12_ != null)
               {
                  ...
                  if(_loc14_ != null)
                  {
                     ...
                     else if(_loc14_.type == AnimationLayerData.const_502)
                     {
                        _loc3_ = _loc11_;
                     }
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1407.getFigure(),_loc5_,this.var_815,_loc3_,_loc6_,_loc7_);
            _loc10_ = new AvatarImageDirectionCache(_loc15_);
            _loc9_.updateDirectionCache(_loc3_,_loc10_);
         }
         this.var_411 = _loc10_.getImageContainer(_loc4_);
         if(this.var_411 == null)
         {
            _loc16_ = _loc10_.getPartList();
            this.var_411 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_385);
            if(this.var_411 == null)
            {
               return null;
            }
            _loc10_.updateImageContainer(this.var_411,_loc4_);
         }
         this.var_411.offset = _loc8_;
         return this.var_411;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_86)
         {
            this.var_86 = this._structure.getCanvas(this._scale,this.var_815);
            if(!this.var_86)
            {
               return null;
            }
            this.var_2713 = new Point(0,this.var_86.height - int(this.var_86.width / 4));
         }
         ...
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_776 || _loc10_ == AvatarAction.const_398;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1520 = this.var_120.bodyPartId;
               this.var_186 = this.var_120.partType;
               this.var_1947 = this.var_120.partId;
               this._frame = this.var_120.getFrameIndex(param3);
               _loc5_ = param1;
               _loc7_ = false;
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_120.flippedPartType != this.var_186)
                     {
                        this.var_186 = this.var_120.flippedPartType;
                     }
                  }
               }
               this.var_1521 = this._scale + "_" + this.var_120.action.assetPartDefinition + "_" + this.var_186 + "_" + this.var_1947 + "_" + _loc5_ + "_" + this._frame;
               this.var_450 = this._assets.getAssetByName(this.var_1521) as BitmapDataAsset;
               if(this.var_450 == null)
               {
                  this.var_1521 = this._scale + "_std_" + this.var_186 + "_" + this.var_1947 + "_" + _loc5_ + "_0";
                  this.var_450 = this._assets.getAssetByName(this.var_1521) as BitmapDataAsset;
               }
               if(!(!this.var_450 || !this.var_450.content))
               {
                  if(this.var_120.paletteMapId > -1)
                  {
                     this.var_937 = this._structure.renderManager.paletteManager.getPalette(this.var_120.paletteMapId);
                     if(this.var_937 != null)
                     {
                        this.var_1015 = this.var_450.content as BitmapData;
                        this.var_185 = new BitmapData(this.var_1015.width,this.var_1015.height);
                        this.var_2714 = this.var_937.colorize(this.var_1015.getPixels(this.var_1015.rect));
                        this.var_185.setPixels(this.var_185.rect,this.var_2714);
                     }
                     else
                     {
                        this.var_185 = (this.var_450.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1520 == AvatarBodyPartType.const_270 || this.var_1520 == AvatarBodyPartType.const_1486 || this.var_1520 == AvatarBodyPartType.const_1563) && this.var_120.color != null)
                  {
                     _loc16_ = 0.7;
                  ...
                  {
                     this.var_185.colorTransform(this.var_185.rect,this.var_120.color.colorTransform);
                  }
                  if(this.var_120.isBlendable)
                  {
                     this.var_185.colorTransform(this.var_185.rect,this.var_120.blendTransform);
                  }
                  this.var_1016 = this.var_450.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_52)
                     {
                  ...
                  this.var_2715 = new ImageData(this.var_185,this.var_1016,_loc7_);
                  this.var_1014.push(this.var_2715);
               }
            }
            _loc8_--;
         }
         ...
         var _loc13_:ImageData = this.createUnionImage(this.var_1014,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2713.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_52)
            {
         ...
         _loc8_ = this.var_1014.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_1014.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
