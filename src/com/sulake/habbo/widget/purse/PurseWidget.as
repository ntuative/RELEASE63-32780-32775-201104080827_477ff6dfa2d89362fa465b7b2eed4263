package com.sulake.habbo.widget.purse
{
   ...
   public class PurseWidget extends RoomWidgetBase
   {
      ...
      private const const_1772:int = 3;
      ...
      public function PurseWidget(param1:IHabboWindowManager, param2:IAssetLibrary, param3:IHabboLocalizationManager)
      {
         ...
         if(_loc4_)
         {
            ...
            this._view.x = _loc5_.width - this._view.width - this.const_1772;
            this._view.y = this.const_1772;
      ...
      {
         this.var_1792 = param1.balance;
         if(this._view)
         {
            this._view.findChildByName("credits").caption = this.var_1792.toString();
            this._view.findChildByName("credits_shadow").caption = this.var_1792.toString();
         }
      }
      ...
      private function onPixelBalance(param1:RoomWidgetPurseUpdateEvent) : void
      {
         ...
         if(this._view)
         {
            this._view.findChildByName("pixels").caption = this.var_1793.toString();
            this._view.findChildByName("pixels_shadow").caption = this.var_1793.toString();
         }
      }
      
      private function onCreditsClick(param1:WindowMouseEvent) : void
      {
         messageListener.processWidgetMessage(new RoomWidgetOpenCatalogMessage(RoomWidgetOpenCatalogMessage.const_1230));
      }
      
      private function onPixelsClick(param1:WindowMouseEvent) : void
      {
         messageListener.processWidgetMessage(new RoomWidgetOpenCatalogMessage(RoomWidgetOpenCatalogMessage.const_1039));
      }
   }
}
