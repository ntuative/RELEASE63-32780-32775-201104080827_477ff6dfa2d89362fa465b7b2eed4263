package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer)
      {
         ...
         this.var_721 = [0,0,0];
         this.var_99 = ["group.walls","group.floors","group.views"];
         super(param1);
         this.var_436 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_436)
         {
            _loc2_ = 0;
            while(_loc2_ < this.var_436.numSelectables)
            {
               _loc3_ = this.var_436.getSelectableAt(_loc2_);
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_99[this.var_581]);
      ...
      {
         var _loc2_:int = this.var_721[this.var_581];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_721[this.var_581] = (this.var_217[this.var_581] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_721[0];
         var _loc2_:int = this.var_721[1];
         var _loc3_:int = this.var_721[2];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         if(configurationAsset != null)
         {
            ...
            {
               if(!(offer.pricingModel == Offer.const_426 || offer.pricingModel == Offer.const_384))
               {
                  continue;
               }
               ...
               if(!(product.productType == ProductTypeEnum.const_74 || product.productType == ProductTypeEnum.const_82))
               {
                  continue;
               }
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         if(_loc2_ > -1)
         {
            ...
            _loc3_ = this.var_721[this.var_581];
            selectIndex(_loc3_);
         }
      }
   }
