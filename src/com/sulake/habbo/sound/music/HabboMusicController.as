package com.sulake.habbo.sound.music
{
   ...
   public class HabboMusicController implements IHabboMusicController, IDisposable
   {
      ...
      private static const const_1409:int = HabboMusicPrioritiesEnum.const_199;
      ...
      public function HabboMusicController(param1:HabboSoundManagerFlash10, param2:IEventDispatcher, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_717.push(new TraxSongInfoMessageEvent(this.onSongInfoMessage));
         this.var_717.push(new UserSongDisksInventoryMessageEvent(this.onSongDiskInventoryMessage));
         for each(_loc5_ in this.var_717)
         {
            this._connection.addMessageEvent(_loc5_);
         }
         this.var_79.addEventListener(RoomEngineSoundMachineEvent.const_676,this.onJukeboxInit);
         this.var_79.addEventListener(RoomEngineSoundMachineEvent.const_277,this.onJukeboxDispose);
         this.var_79.addEventListener(RoomEngineSoundMachineEvent.const_915,this.onSoundMachineInit);
         this.var_79.addEventListener(RoomEngineSoundMachineEvent.const_722,this.onSoundMachineDispose);
         this.var_1250 = new Timer(1000);
         this.var_1250.start();
         this.var_1250.addEventListener(TimerEvent.TIMER,this.sendNextSongRequestMessage);
      ...
      protected function onSongFinishedPlayingEvent(param1:SoundCompleteEvent) : void
      {
         ...
         {
            if(this.getTopRequestPriority() == this._priorityPlaying && this.getSongRequestCountAtPriority(this._priorityPlaying) == this.var_1926)
            {
               this.resetSongStartRequest(this._priorityPlaying);
            }
            _loc2_ = this._priorityPlaying;
            this.playSongWithHighestPriority();
            if(_loc2_ >= HabboMusicPrioritiesEnum.const_370)
            {
               this._events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_586,_loc2_,param1.id,-1));
            }
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(this._connection)
            {
               for each(_loc1_ in this.var_717)
               {
            ...
            {
               _loc2_ = 0;
               while(_loc2_ < this.var_446.length)
               {
                  _loc3_ = this.var_446.getWithIndex(_loc2_) as SongDataEntry;
            ...
            if(this.var_79)
            {
               this.var_79.removeEventListener(RoomEngineSoundMachineEvent.const_676,this.onJukeboxInit);
               this.var_79.removeEventListener(RoomEngineSoundMachineEvent.const_277,this.onJukeboxDispose);
               this.var_79.removeEventListener(RoomEngineSoundMachineEvent.const_915,this.onSoundMachineInit);
               this.var_79.removeEventListener(RoomEngineSoundMachineEvent.const_722,this.onSoundMachineDispose);
            }
      ...
      private function getSongDataEntry(param1:int) : SongDataEntry
      {
         ...
         {
            _loc2_ = this.var_446.getValue(param1) as SongDataEntry;
         }
         return _loc2_;
      }
      ...
      {
         if(param1 >= 0 && param1 < this.var_380.length)
         {
            return this.var_380.getKey(param1);
         }
         return -1;
      }
      ...
      public function getSongDiskInventorySongId(param1:int) : int
      {
         if(param1 >= 0 && param1 < this.var_380.length)
         {
            return this.var_380.getWithIndex(param1);
         }
         return -1;
      }
      ...
      private function sendNextSongRequestMessage(param1:TimerEvent) : void
      {
         ...
         this._connection.send(new GetSongInfoMessageComposer(this.var_809));
         Logger.log("Requested song info\'s : " + this.var_809);
         this.var_809 = new Array();
      }
      ...
      private function onSongInfoMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc5_ < _loc4_.length)
         {
            ...
            if(_loc7_)
            {
               ...
               this.var_446.add(_loc6_.id,_loc10_);
               ...
               while(this.var_810.indexOf(_loc6_.id) != -1)
               {
                  this.var_810.splice(this.var_810.indexOf(_loc6_.id),1);
               }
               if(this.var_810.length == 0)
               {
                  this._events.dispatchEvent(new SongDiskInventoryReceivedEvent(SongDiskInventoryReceivedEvent.const_551));
               }
      ...
      private function playSongObject(param1:int, param2:int) : Boolean
      {
         ...
         this.var_1926 = this.getSongRequestCountAtPriority(param1);
         this.var_1925 = param2;
         if(this._priorityPlaying <= const_1409)
         {
            this.notifySongPlaying(_loc4_);
         }
         if(param1 > HabboMusicPrioritiesEnum.const_199)
         {
      ...
      private function notifySongPlaying(param1:SongDataEntry) : void
      {
         ...
         if(param1.length >= 8000 && (this.var_2692 != param1.id || _loc3_ > this.var_2691 + 8000))
         {
            this._soundManager.notifyPlayedSong(param1.name,param1.creator);
            this.var_2692 = param1.id;
      ...
      private function onSongDiskInventoryMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDiskCount)
         {
            ...
            if(this.var_446.getValue(_loc6_) == null)
            {
               this.var_810.push(_loc6_);
